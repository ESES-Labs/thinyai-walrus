...
`sui_transfer` and `sui_move_call` build the PTB locally and run it through the same gated path
(simulate → policy → approve → sign), so the agent can transact without an external builder. The
signer may be passed as a value or a **getter** (`() => SuiSigner | null`) for runtime configuration.
